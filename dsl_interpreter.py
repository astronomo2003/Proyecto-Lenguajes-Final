...
class DSLInterpreter(DeepLearningDSLBaseVisitor):
    ...
    def visitPlot_function(self, ctx):
        ...
        try:
            print(f"DEBUG: visitPlot_function llamado con ctx: {type(ctx)}")
            
            if ctx.PLOT():
                print("DEBUG: Procesando PLOT")
                data = self.visit(ctx.expression())
                print(f"DEBUG: Datos obtenidos para plot: {data} (tipo: {type(data)})")
                result = self.plotter.plot(data)
                print(f"DEBUG: Resultado del plot: {result}")
                return result
                
            elif ctx.SCATTER():
                print("DEBUG: Procesando SCATTER")
                x_data = self.visit(ctx.expression(0))
                y_data = self.visit(ctx.expression(1))
                print(f"DEBUG: Datos scatter - x: {x_data}, y: {y_data}")
                return self.plotter.scatter(x_data, y_data)
                
            elif ctx.HISTOGRAM():
                print("DEBUG: Procesando HISTOGRAM")
                data = self.visit(ctx.expression())
                print(f"DEBUG: Datos histogram: {data}")
